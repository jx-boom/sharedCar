...
<script>
 ...
 var that= null;
  export default {
    ...
    onShow() {
      http.defLocateInfo( (res) => {
        if(res.code==0){

          wx.downloadFile({
            url: 'https://app.ymai.net.cn/static/APPimage/online.png',
            success: function(resFile) {
              // 只要服务器有响应数据，就会把响应内容写入文件并进入 success 回调，业务需要自行判断是否下载到了想要的内容
              console.log(resFile);
              console.log("服务器有响应数据");
              if (resFile.statusCode === 200) {
                that.iconPath= resFile.tempFilePath;
                let circles= [];
                let markers= [];
                markers.push({
                  id: res.data.id,
                  ...
                  iconPath :resFile.tempFilePath,
                  name: 'name'
                });
                circles.push(
                  {
                    "longitude": res.data.rolLongitude,
                    "latitude": res.data.rolLatitude,
                    "radius": res.data.radius*1000,
                    'color': '#13ec2b82',
                    'fillColor':'#13ec2b21',
                  })
                that.circles=circles;
                that.markers= markers;
                that.longitude=  res.data.rolLongitude;
                that.latitude= res.data.rolLatitude;
    ...
    mounted () {
      ...
      //               that.latitude= res.data[0]['rolLatitude'];
      //               that.mapCtx.moveToLocation();
      //               if(res.data.length==0){
      //                 wx.getLocation({
      //                   type: 'wgs84',
            ...
            that.circles = [
              {
                "longitude":  res.data.rolLongitude,
                "latitude": res.data.rolLatitude,
                "radius":  res.data.radius?res.data.radius*1000:0,
                'color': '#13ec2b82',
                'fillColor':'#13ec2b21',
              }
            ];
